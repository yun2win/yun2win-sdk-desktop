...
$.widget( "evol.colorpicker", {
	...
	options: {
		...
		strings: '标准色彩,基本色彩,Web Colors,Theme Colors,Back to Palette,History,No history yet.'
	},
		...
		switch(this.element.get(0).tagName){
			case 'INPUT':
				...
				if(showOn==='both' || showOn==='button'){
					e.next().on('click touchmove', function(evt){
						evt.stopPropagation();
						that.showPalette();
						return false;
					});
	...
	_bindColors: function() {
		...
		this._palette
			.on('click touchmove', sel, function(evt){
                if(that._enabled){
					var $this=$(this);
